...
def benchmark_npu(iterations=1000, seed=42, prefix="", compute_unit=ct.ComputeUnit.ALL, name="ALL"):
    np.random.seed(seed)
    ...
    filename = f"results/{filename_prefix}npu_stats_{name}_{int(time.time())}.json"
...
if __name__ == "__main__":
    ...
    benchmark_npu(iterations=args.runs, seed=args.seed, compute_unit=ct.ComputeUnit.CPU_ONLY, name="CPU_ONLY")
    # Run with CPU AND GPU to track GPU performance impact within CoreML
    benchmark_npu(iterations=args.runs, seed=args.seed, compute_unit=ct.ComputeUnit.CPU_AND_GPU, name="CPU_AND_GPU")
    # Run with ALL (Which forces ANE if available)
    benchmark_npu(iterations=args.runs, seed=args.seed, compute_unit=ct.ComputeUnit.ALL, name="ALL")
