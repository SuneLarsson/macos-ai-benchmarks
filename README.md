...
1. **Apple Neural Engine (NPU) validation:** Uses `coremltools` to target the ANE. On `macos-exec-kubelet`, this runs with blistering speed. On `macOS-vz-kubelet`, the NPU is unavailable, triggering an automatic fallback to the CPU/GPU resulting in measurable latency drops. Another configuration setting (`ct.ComputeUnit.CPU_AND_NE`) could be used (NPU and CPU) to see if the user-space implementation can access the NPU.
...
4. **Cross-Platform PyTorch Benchmark:** Measures hardware-agnostic cross-platform tensor operations using PyTorch.
5. **Cross-Platform LLM Inference:** Uses `mlx-lm` and `transformers` to benchmark LLM generation speeds.
6. **NFS Data Pipeline:** Measures the streaming and preloading latency using MLX/PyTorch directly.
7. **MLX Benchmark Suite:** Runs TristanBilot's suite to measure micro-profiling of MLX operations.
8. **Storage Overhead (Model/Dataset Loading):** Uses `fio` to simulate the network latency and throughput of reading large model weights and datasets over NFS.
...
### 4. Parsing and Plotting Results

The `scripts/` directory contains various Python tools used to parse the raw output files generated by the benchmarks and create visualization plots or LaTeX tables.
