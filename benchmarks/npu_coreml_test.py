...
def benchmark_npu(iterations=1000, seed=42, prefix="", compute_unit=ct.ComputeUnit.ALL, name="ALL", output_dir="results"):
    np.random.seed(seed)
    ...
    filename = f"{output_dir}/{filename_prefix}npu_stats_{name}_{time.strftime('%Y-%m-%d-%H-%M')}.json"
...
if __name__ == "__main__":
    ...
    benchmark_npu(iterations=args.runs, seed=args.seed, compute_unit=ct.ComputeUnit.CPU_ONLY, name="CPU_ONLY", output_dir=args.output_dir)
    # Run with CPU AND GPU to track GPU performance impact within CoreML
    benchmark_npu(iterations=args.runs, seed=args.seed, compute_unit=ct.ComputeUnit.CPU_AND_GPU, name="CPU_AND_GPU", output_dir=args.output_dir)
    # Run with ALL (Which forces ANE if available)
    benchmark_npu(iterations=args.runs, seed=args.seed, compute_unit=ct.ComputeUnit.ALL, name="ALL", output_dir=args.output_dir)
